...
# -*- coding: utf-8 -*-
"""
ROS2 기반 드론 최신 기술 일일 수집 (2026-08-23)
- GitHub Search API (9 쿼리, per_page=10, sort=stars desc)
- PX4 / ArduPilot 릴리즈 노트 (GitHub Releases API)
- PX4 공식 문서 (docs.px4.io/main/en/ — key pages)
- ROS2 공식 문서 (docs.ros.org/en/rolling/ — key pages)
각 raw/articles/YYYY-MM-DD-*.md 파일에 frontmatter + sha256(body) 저장.
Gate B 검증용: sha256는 frontmatter 닫힌 --- 이후 body 전체 바이트 해시.
"""
import json
import hashlib
import os
...
UA = ("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) "
      ...
      "Chrome/127.0 Safari/537.36 HermesAgent/1.0")
...
PX4_PAGES = [
    ...
    ("https://docs.px4.io/main/en/computer_vision/", "Computer Vision (Optical Flow, MoCap, VIO, Avoidance)"),
    ("https://docs.px4.io/main/en/releases/main.html", "PX4 Main Release Notes"),
    ("https://docs.px4.io/main/en/ros2/", "ROS 2 Interface (px4_ros_com)"),
    ("https://docs.px4.io/main/en/middleware/zenoh.html", "Zenoh Middleware"),
]

# ROS2 Rolling docs to fetch
ROS2_PAGES = [
    ...
    ("https://docs.ros.org/en/rolling/Concepts/About-ROS-2-Architecture.html", "ROS 2 Concepts"),
    ("https://docs.ros.org/en/rolling/Concepts/Basic/Quality-of-Service.html", "Quality of Service (QoS)"),
    ("https://docs.ros.org/en/rolling/Instructions/Demos/Topics/Topics-demo.html", "Navigation 2 (Nav2)"),
]

PX4_RELEASES_API = "https://api.github.com/repos/PX4/PX4-Autopilot/releases"
AP_RELEASES_API = "https://api.github.com/repos/ArduPilot/ardupilot/releases"


def fetch_json(url, is_search=False):
    """GitHub API JSON fetcher with optional auth token."""
...
def strip_html(html):
    """Extract meaningful text from HTML, removing script/style/noscript."""
...
def save_raw_article(filename, body, source_url, title, extra_fields=""):
    """
    Save a raw article file in the exact 2026-08-22 format.
    Frontmatter order: source_url, ingested, title, captured_via, [extras], sha256
    sha256 = SHA-256 of the exact post-frontmatter body bytes.
    Normalize CRLF -> LF so text-mode reads in verification stay consistent.
    """
    body = body.replace("\r\n", "\n").replace("\r", "\n")
    ...
    fm_parts.append(f'sha256: {sha256}')
    frontmatter = "---\n" + "\n".join(fm_parts) + "\n---\n"
...
def main():
    ...
    gh_header = f"# ROS2 드론 관련 GitHub 저장소 검색 결과 ({TODAY})\n\n"
    ...
    for rank, it in enumerate(unique_items[:10], 1):
        ...
        desc_item = (it.get("description") or "(none)")[:140]
    ...
    if not gh_body.endswith("\n"):
        gh_body += "\n"

    filename = f"{TODAY}-ros2-drone-github-data.md"
    sha256 = save_raw_article(
        filename, gh_body, "GitHub Search API (9 queries)",
        f"ROS2 드론 GitHub 검색 데이터 ({TODAY})",
        f'search_queries: "9 queries, per_page=10, sort=stars desc"'
    )
    ...
    try:
        ...
        for r in releases:
            ...
            if body_text:
                ...
                px4_rel_body += "\n\n"
        if not px4_rel_body.endswith("\n"):
            px4_rel_body += "\n"
        filename = f"{TODAY}-px4-release-notes.md"
        sha256 = save_raw_article(filename, px4_rel_body, PX4_RELEASES_API, f"PX4 릴리즈 노트 ({TODAY})")
        print(f"  ✓ {filename} (sha256: {sha256[:16]}…)")
        results[filename] = {"sha256": sha256, "url": PX4_RELEASES_API}
    except Exception as e:
    ...
    try:
        ...
        for r in releases:
            ...
            if body_text:
                ...
                ap_rel_body += "\n\n"
        if not ap_rel_body.endswith("\n"):
            ap_rel_body += "\n"
        filename = f"{TODAY}-ardupilot-release-notes.md"
        sha256 = save_raw_article(filename, ap_rel_body, AP_RELEASES_API, f"ArduPilot 릴리즈 노트 ({TODAY})")
        print(f"  ✓ {filename} (sha256: {sha256[:16]}…)")
        results[filename] = {"sha256": sha256, "url": AP_RELEASES_API}
    except Exception as e:
    ...
    px4_docs_body += "수집 방법: docs.px4.io/main/en/ (curl + HTML 텍스트 추출, 2nd-brain-cron)\n\n"
    px4_docs_body += "".join(px4_docs_parts)
    if not px4_docs_body.endswith("\n"):
        px4_docs_body += "\n"
    filename = f"{TODAY}-px4-docs.md"
    sha256 = save_raw_article(
        filename, px4_docs_body, "https://docs.px4.io/main/en/",
        f"PX4 공식 문서 수집 ({TODAY})",
    ...
    ros2_docs_body += "수집 방법: docs.ros.org/en/rolling/ (curl + HTML 텍스트 추출, 2nd-brain-cron)\n\n"
    ros2_docs_body += "".join(ros2_docs_parts)
    if not ros2_docs_body.endswith("\n"):
        ros2_docs_body += "\n"
    filename = f"{TODAY}-ros2-docs.md"
    sha256 = save_raw_article(
        filename, ros2_docs_body, "https://docs.ros.org/en/rolling/",
        f"ROS2 공식 문서 수집 ({TODAY})",
    ...
    Path(f"{REPO}/docs/workflow").mkdir(parents=True, exist_ok=True)
    ...
    # Save GitHub data summary for report generation
    summary = {
        "top_repos": [
            {
                "full_name": it.get("full_name", ""),
        ...
        "query_counts": {q[0]: q[3] for q in query_results},
    ...
    return results, unique_items, query_results
